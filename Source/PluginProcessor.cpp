...
void SimpleSynthAudioProcessor::keyOn(int pitch, int velocity)
{
    dspFaust.keyOn(pitch,velocity);
}

void SimpleSynthAudioProcessor::keyOff(int pitch)
{
    dspFaust.keyOff(pitch);
}

void SimpleSynthAudioProcessor::setCutoff(float cutoff)
{
    dspFaust.setParamValue("/Sequencer/DSP1/Polyphonic/Voices/SimpleSynth/fc",cutoff);
}
